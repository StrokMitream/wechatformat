import { stripUnresolvedAsyncPlaceholders, waitForPreviewReady } from '@/lib/preview-ready'
...
export interface ProcessClipboardHooks {
  /** Re-render the preview in light theme before copying (WeChat is light-only). */
  renderLight?: () => void
  /** Restore the preview to its previous theme after copying. */
  restore?: () => void
}
 ...
 */
export async function processClipboardContent(primaryColor: string, hooks?: ProcessClipboardHooks) {
